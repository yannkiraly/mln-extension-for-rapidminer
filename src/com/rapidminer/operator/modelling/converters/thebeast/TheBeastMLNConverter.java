...
public class TheBeastMLNConverter extends MLNFileConverter {
	private TheBeastMLNParseListener parseListener;
	...
	public TheBeastMLNConverter(InputStream inputStream, OutputStream outputStream) {
	...
	public MLNModel internalBuildModel(MLNFileRole role) throws MLNException{
		parseListener = new TheBeastMLNParseListener();
		ParseTree tree;
		try {
	...
	protected int buildTypeDeclarations(MLNModel mlnModel)  throws MLNException  {
		...
		for (MLNTypeContainer typeContainer : list) {
			write(TheBeastMLNBuilder.buildTypeDeclaration(typeContainer)+"\n");
		}
		return list.size();
	}
	...
	protected int buildPredicateDeclarations(MLNModel mlnModel)  throws MLNException {
		...
		for (MLNPredicate predicate : list) {
			write(TheBeastMLNBuilder.buildPredicateDeclaration(predicate)+"\n");
		}
		return list.size();
	}
	...
	protected int buildFunctionDeclarations(MLNModel mlnModel)  throws MLNException  {
		...
		for (MLNFunction function : list) {
			write(TheBeastMLNBuilder.buildFunctionDeclaration(function)+"\n");
		}
		return list.size();
	}
	...
	protected int buildGroundFormulas(MLNModel mlnModel)  throws MLNException  {
		...
		for (MLNGroundFormula groundFormula : list) {
			write(TheBeastMLNBuilder.buildGroundFormula(groundFormula)+"\n");
		}
		return list.size();
	}
	...
	protected int buildProbabilityStatements(MLNModel mlnModel) throws MLNException  {
		...
		for (MLNProbabilityStatement statement: list) {
			write(TheBeastMLNBuilder.buildProbablityStatement(statement)+"\n");
		}
		return list.size();
	}
	...
	protected int buildFormulas(MLNModel mlnModel) throws MLNException {
		...
		for (MLNFormula formula : list) {
			write(TheBeastMLNBuilder.buildFormula(formula, convertLEconstraints, removeUnsupportedConstraints)+"\n");
		}
		return list.size();
	}
