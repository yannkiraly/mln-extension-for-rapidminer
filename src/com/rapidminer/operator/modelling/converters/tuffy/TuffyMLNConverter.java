...
import com.rapidminer.operator.modelling.converters.rockit.RockitMLNConverter;
import com.rapidminer.operator.modelling.converters.tuffy.parser.TuffyMLNGrammarLexer;
...
public class TuffyMLNConverter extends MLNFileConverter {
	public TuffyMLNConverter(InputStream inputStream, OutputStream outputStream) {
		super(inputStream, outputStream);
	}

	private TuffyMLNParseListener parseListener;
	private ANTLRInputStream inputData;
	private OutputStreamWriter outputStreamWriter;
	private TuffyMLNBuilder tuffyBuilder;
	...
	private TokenStream getTokenStream() {
		TuffyMLNGrammarLexer lexer =  new TuffyMLNGrammarLexer(inputData);
		return new CommonTokenStream(lexer);
	}
	
	private TuffyMLNGrammarParser getParser(TokenStream tokenStream) {
		return new TuffyMLNGrammarParser(tokenStream);
	}
	
	@Override
	public MLNModel internalBuildModel(MLNFileRole role) throws MLNException{
		parseListener = new TuffyMLNParseListener();
		ParseTree tree;
		try {
	...
	protected int buildTypeDeclarations(MLNModel mlnModel) throws MLNException {
		...
		for (MLNTypeContainer typeContainer : list) {
			write(tuffyBuilder.buildTypeDeclaration(typeContainer)+"\n");
		}
		return list.size();
	}
	...
	protected int buildPredicateDeclarations(MLNModel mlnModel) throws MLNException {
		...
		for (MLNPredicate predicate : list) {
			write(tuffyBuilder.buildPredicateDeclaration(predicate)+"\n");
		}
		return list.size();
	}
	
	protected int buildQueryPredicateDeclarations(MLNModel mlnModel) throws MLNException {
		List<MLNPredicate> list =  mlnModel.getAll(MLNPredicate.class);
		for (MLNPredicate predicate : list) {
			write(tuffyBuilder.buildQueryPredicateDeclaration(predicate)+"\n");
		}
		return list.size();
	}
	...
	protected int buildFunctionDeclarations(MLNModel mlnModel) throws MLNException {
		...
		for (MLNFunction function : list) {
			write(tuffyBuilder.buildFunctionDeclaration(function)+"\n");
		}
		return list.size();
	}
	...
	protected int buildGroundFormulas(MLNModel mlnModel) throws MLNException {
		...
		for (MLNGroundFormula groundFormula : list) {
			write(tuffyBuilder.buildGroundFormula(groundFormula)+"\n");
		}
		return list.size();
	}
	...
	protected int buildProbabilityStatements(MLNModel mlnModel) throws MLNException {
		...
		for (MLNProbabilityStatement statement: list) {
			write(tuffyBuilder.buildProbablityStatement(statement)+"\n");
		}
		return list.size();
	}
	...
	protected int buildFormulas(MLNModel mlnModel) throws MLNException {
		...
		for (MLNFormula formula : list) {
			write(tuffyBuilder.buildFormula(formula, convertLEconstraints, removeUnsupportedConstraints)+"\n");
		}
		return list.size();
	}
	...
	public void internalConvertModel(MLNModel model, MLNFileRole role)
			throws MLNException {
		updateFloatType(model,RockitMLNConverter.FLOAT_TYPE_NAME, false, Double.MIN_VALUE, Double.MAX_VALUE);
		outputStreamWriter = new OutputStreamWriter(outputStream);
		tuffyBuilder = new TuffyMLNBuilder(model);
		if (role.equals(MLNFileRole.RULES)) {
			...
			buildQueryPredicateDeclarations(model);
